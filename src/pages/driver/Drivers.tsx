...
const Drivers: React.FC = () => {
 ...
 const filteredDrivers = driversData.data.filter((driver) => {
  ...
  const matchesVehicleSize = !vehicleSizeFilter || 
    driver.vehicleSize.toString() === vehicleSizeFilter;
  
  return matchesSearch && matchesLicense && matchesStatus && matchesVehicleType && matchesVehicleSize;
});
...
const getVehicleTypeArabic = (type: "Standard" | "Vip" | "Premium" | string) => {
  const types = {
    Standard: "عادي",
    Vip: "مميز",
    Premium: "فاخر",
  ...
  useEffect(() => {
    setCurrentPage(1);
  }, [searchTerm, statusFilter, vehicleTypeFilter, vehicleSizeFilter,licenseFilter]);
